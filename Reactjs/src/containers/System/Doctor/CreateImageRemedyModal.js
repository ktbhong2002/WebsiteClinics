...
class CreateImageRemedyModal extends Component {
  ...
  handleFilterDrugs=(event)=>{
      ...
      temp = temp.filter((drug)=>drug.name.toUpperCase().indexOf(event.target.value.toUpperCase()) > -1)
      ...
      >
        ...
        <ModalBody>
          <div className="row">
                          ...
                          return(
                              <div class="col-12 form-group">
                                <div class="row align-item-center text-center">
                                    ...
                                    <div class="col-auto">
                                        <select class="form-control" onChange={(event)=>this.handleOnchangeUnitDrug(event,drug.id)} value={drug.unit}>
                                    ...
                                    <div class="col-5"> <input onChange={(event)=>this.handleOnchangeDescriptionUsageDrug(event,drug.id)} type="text" value={drug.desciption} class="form-control" placeholder="Enter desription" /></div>
                                    <div class="col-1 d-flex align-items-center"><i onClick={()=>this.removeDrugFromTheList(drug.id)} class="fas fa-trash pointer text-red"></i></div>
                                </div>
                              </div>
                          );
                        })
            ...
            <div class="col-4">
                    <div class="row">
                        <div class="col-12">Danh sách thuốc
                          ...
                          <ul class="list-group mt-10" style={{overflowY: "scroll", maxHeight: "400px"}}>
                            {
                              listFilterDrugs.map((drug)=>{
                                return(
                                  <li onClick={()=>this.handlePushDrugToList(drug)} class="list-group-item list-group-item-action d-flex justify-content-between align-items-center pointer"><span>{drug.name}</span> <i class="text-primary fas fa-plus-circle"></i></li>
                                )
                              })
                            }
                          </ul>
