...
function useAutocomplete(props) {
  ...
  const ignoreFocus = React3.useRef(false);
  const firstFocus = React3.useRef(true);
  const inputRef = React3.useRef(null);
  const listboxRef = React3.useRef(null);
  const [anchorEl, setAnchorEl] = React3.useState(null);
  const [focusedTag, setFocusedTag] = React3.useState(-1);
  const defaultHighlighted = autoHighlight ? 0 : -1;
  const highlightedIndexRef = React3.useRef(defaultHighlighted);
  ...
  const resetInputValue = React3.useCallback((event, newValue) => {
  ...
  const [inputPristine, setInputPristine] = React3.useState(true);
  ...
  const syncHighlightedIndex = React3.useCallback(() => {
    if (!popupOpen) {
      return;
    }
  ...
  const isTouch = React3.useRef(false);
...
function useSnackbar(parameters = {}) {
  ...
  const handleResume = React5.useCallback(() => {
...
var Badge = React6.forwardRef(function Badge2(props, forwardedRef) {
  ...
  return (0, import_jsx_runtime5.jsxs)(Root, _extends({}, rootProps, {
    children: [children, (0, import_jsx_runtime4.jsx)(BadgeComponent, _extends({}, badgeProps, {
...
true ? Button.propTypes = {
   ...
   * @default 'button'
   */
  rootElementName: import_prop_types4.default.string,
  /**
...
function Dropdown(props) {
  ...
  return (0, import_jsx_runtime7.jsx)(DropdownContext.Provider, {
    value: contextValue,
    children
  });
}
...
var TransitionContext = React27.createContext(null);
if (true) {
...
function useTransitionStateManager() {
  const transitionContext = React28.useContext(TransitionContext);
...
function useTransitionTrigger(requestEnter) {
  const [exitTransitionFinished, setExitTransitionFinished] = React29.useState(true);
  const hasPendingExitTransition = React29.useRef(false);
  const registeredTransitions = React29.useRef(0);
  const [hasTransition, setHasTransition] = React29.useState(false);
  const previousRequestEnter = React29.useRef(requestEnter);
  React29.useEffect(() => {
  ...
  const handleExited = React29.useCallback(() => {
    hasPendingExitTransition.current = false;
    setExitTransitionFinished(true);
  }, []);
  const registerTransition = React29.useCallback(() => {
...
var PopupContext = React30.createContext(null);
if (true) {
...
var Popup = React31.forwardRef(function Popup2(props, forwardedRef) {
  var _slots$root;
  const {
    anchor: anchorProp,
    children,
  ...
  const popupContextValue = React31.useMemo(() => ({
    placement: finalPlacement
  }), [finalPlacement]);
...
function useMenuItemContextStabilizer(id) {
  ...
  const localGetItemState = React36.useCallback((itemValue) => {
    if (itemValue !== itemId) {
  ...
  const localContextValue = React36.useMemo(() => ({
    dispatch,
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-A4KWYDQ3.js.map
