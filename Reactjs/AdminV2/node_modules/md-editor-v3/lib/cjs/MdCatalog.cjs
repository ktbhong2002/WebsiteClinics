...
const CatalogLink = /* @__PURE__ */ vue.defineComponent({
  props: props$1,
  setup(props2) {
    return () => {
      const {
      ...
      return vue.createVNode("div", {
        "class": [`${config.prefix}-catalog-link`, tocItem.active && `${config.prefix}-catalog-active`],
        "onClick": (e) => {
          onClick(e, tocItem);
          e.stopPropagation();
        ...
        "title": tocItem.text
      }, [tocItem.text]), vue.createVNode("div", {
        "class": `${config.prefix}-catalog-wrapper`
      }, [tocItem.children && tocItem.children.map((item) => vue.createVNode(CatalogLink, {
...
const MdCatalog = /* @__PURE__ */ vue.defineComponent({
  name: "MdCatalog",
  props,
  ...
  setup(props2, ctx) {
    ...
    const activeItem = vue.shallowRef();
    const catalogs = vue.computed(() => {
      const tocItems = [];
    ...
    const findActiveHeading = (list) => {
      ...
      const {
        activeHead
      } = list.reduce((activeData, link, index$1) => {
        const linkEle = document.getElementById(props2.mdHeadingId(link.text, link.level, index$1 + 1));
        if (linkEle instanceof HTMLElement) {
          const scrollElement = getScrollElement();
          const relativeTop = index.getRelativeTop(linkEle, scrollElement);
    ...
    vue.watch(() => activeItem.value, (nVal) => {
    ...
    vue.onMounted(() => {
      ...
      index.bus.emit(editorId, index.PUSH_CATALOG);
    });
    vue.onBeforeUnmount(() => {
      var _a;
      const scrollElement = getScrollElement();
    ...
    return () => vue.createVNode("div", {
      "class": `${config.prefix}-catalog${props2.theme === "dark" ? "-dark" : ""} ${props2.class}`
    }, [catalogs.value.map((item) => {
      return vue.createVNode(CatalogLink$1, {
