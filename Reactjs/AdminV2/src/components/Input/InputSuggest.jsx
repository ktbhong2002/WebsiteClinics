...
class InputSuggestion extends React.Component {
  ...
  getSuggestions = (textInput) => {
    ...
    const { inputsWithIndex } = this.props;
    const keyArr = Object.keys(inputsWithIndex).filter(
      (textInput) => textInput.toLowerCase().indexOf(inputValue) >= 0
    );
  ...
  renderSuggestion = (suggestion) => <div className="suggest-item">{suggestion.displayName}</div>;
  ...
  sortSuggestions(suggestions, value) {
    const results = _.sortBy(suggestions, (element) => element.displayName);
    return results;
  }
